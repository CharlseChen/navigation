...
<section class="content-header">
    <h1>
        站点设置
        <small>站点信息设置@if (!$checkCacheConf) <span class="form-group has-error"><label>(缓存未生成,请保存下生成缓存)</label></span> @endif </small>
    </h1>
    <ol class="breadcrumb">
                ...
                <form role="form" name="addForm" id="website_form" method="POST" action="{{ route('admin.setting.website') }}">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group @if ($errors->has('website_url')) has-error @endif">
                            ...
                            <span class="text-muted">(您站点的完整域名。例如: http://www.qdefense.cn，不要以斜杠 (“/”) 结尾)</span>
                        ...
                        <div class="form-group @if ($errors->has('website_version')) has-error @endif">
                            <label for="website_admin_page">系统软件版本号</label>
                            <span class="text-muted">(默认从1.0开始)</span>
                            <input type="text" name="website_version" class="form-control " placeholder="系统软件版本号" value="{{ old('website_version',Setting()->get('website_version')) }}">
                            @if ($errors->has('website_version')) <p class="help-block">{{ $errors->first('website_version') }}</p> @endif
                        </div>
...
--}}
                        <div class="form-group @if ($errors->has('website_admin_email')) has-error @endif">
                            ...
                            <span class="text-muted">(设置范围是1-8640，缓存相关数据等)</span>
                       ...
                       <div class="form-group @if ($errors->has('website_admin_page')) has-error @endif">
                            <label for="website_admin_page">系统每页显示条数</label>
                            <span class="text-muted">(设置范围是1-50)</span>
                            <input type="text" name="website_admin_page" class="form-control " placeholder="系统每页的显示条数" value="{{ old('website_admin_page',Setting()->get('website_admin_page')) }}">
                            @if ($errors->has('website_admin_page')) <p class="help-block">{{ $errors->first('website_admin_page') }}</p> @endif
                        </div>
