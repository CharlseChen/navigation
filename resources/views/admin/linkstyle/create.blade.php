    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <div class="box box-default">
                <form role="form" name="userForm" method="POST" action="{{ route('admin.style.store') }}"  enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <div class="box-body">
                        <div class="form-group @if ($errors->has('class_name')) has-error @endif">
                            <label>样式名称(<b style="color:red">注:新增样式前,请更新/public/css/web_mark.css文件</b>)</label>
                            <input type="text" name="class_name" class="form-control " required oninvalid="setCustomValidity('必须填写网站名称!');"  oninput="setCustomValidity('')" placeholder="请输入链接样式类名" value="{{old('class_name')}}">
                            @if ($errors->has('web_name')) <p class="help-block">{{ $errors->first('class_name') }}</p> @endif
                        </div>
                        <div>
                            <label>样式展示:</label>
                            <a class="show_style" href="javascript:void(0);">量子防务</a>
                        </div>
                    </div>
