...
class UserController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        $users = $query->orderBy('created_at','desc')->paginate(Config::get('game.admin.page_size'));
     ...
     * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector|\Illuminate\Routing\Redirector|\Illuminate\Http\RedirectResponse
     */
    public function chpass(Request $request)
    {
        ...
        if($request->isMethod('POST')){
            ...
            if(Hash::check($request->input('old_password'),$user->password)){
                ...
                $user->save();
                Auth()->logout();
                return $this->success(route('admin.account.login'),'密码修改成功,请重新登录');
            }
            
            return redirect(route('admin.account.chpass'))
            ->withErrors([
                'old_password' => '原密码错误！',
            ]);
        }
